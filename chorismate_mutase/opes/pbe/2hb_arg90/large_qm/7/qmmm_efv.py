...
pseudo_bond_param_dir = f"{home_dir}/projects/pseudo_bond/jcp2008/refined_params/wb97xv/631Gss_separate"
aspc_nvec = 4
...
#    assert abs(np.sum(mm_charges) + charge) < 1e-8
    ...
    if auxbasis is not None:
        for ele, bas in auxbasis.items():
            auxbas[ele] = bas
    mf = RKS(mol, xc=xc).density_fit(auxbasis=auxbas)
